...
class Yolov7Publisher:
    def __init__(self, img_topic: str, weights: str, conf_thresh: float = 0.5,
        ...
        :param frequency allow to set an update rate to the to slow it down
        """
        ...
        self.img_subscriber = rospy.Subscriber(
            img_topic, Image, self.on_receive_img_msg
        )
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.img_buffer is not None:
                self.process_img_msg(self.img_buffer)
            t_duration = time.time()-t_start
            t_sleep    = sleep_time - t_duration
            if t_sleep > 0:
                rospy.sleep(t_sleep)

    def on_receive_img_msg(self, img_msg: Image):
        self.img_buffer = img_msg
...
if __name__ == "__main__":
    ...
    try:
        ...
        frequency = rospy.get_param(ns + "frequency")
    except KeyError as e:
        rospy.logerr(f"[yolov7-ros] Error on init, missing parameter named {e} !")
        sys.exit()
